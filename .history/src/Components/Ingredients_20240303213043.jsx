...
export default function Ingredients() {
    ...
    return (
        <div>
            <div className='row '>
                {data?.data?.meals !== null && data.data.meals.map((meal) => (
                    <div className="col-xl-3 col-lg-6 text-white mt-5" key={meal.idIngredient}>
                        <Link to={`/ingredients/${meal.strIngredient}`} className=' text-decoration-none text-white'>
                            <div className="img text-center">
                                <FaDrumstickBite className='fs-90'/>
                                <h3 className=' mt-2 '>{meal.strIngredient}</h3>
                                <p>{meal.strDescription && getFirst10Words(meal.strDescription)}</p>
                            </div>
                        </Link>
                    </div>
                ))}

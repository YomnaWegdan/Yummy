...
export default function Search() {
    ...
    const { data: nameData, isLoading: nameIsLoading, refetch: refetchSearchName } = useQuery("mealsBySearch", () => nameQuery ? getMealsBySearch(nameQuery) : undefined);
    const { data: letterData, isLoading: letterIsLoading, refetch: refetchSearchLetter } = useQuery("mealsBySearchByFName", () => letterQuery ? getMealsSearchByFName(letterQuery) : undefined);
    ...
    return (
        <div>
            <div className="row min-vh-100">
                ...
                {letterQuery && letterData?.data?.meals?.map((meal) => (
                    <MealsItem key={meal.idMeal} meal={meal} />
                ))}

                {nameQuery && nameData?.data?.meals?.map((meal) => (
                    <MealsItem key={meal.idMeal} meal={meal} />
                ))}
